...
* [`src/reframe_app/`](src/reframe_app/): SPA source files (ClojureScript,
[re-frame](https://github.com/Day8/re-frame))
  - [`core.cljs`](src/reframe_app/core.cljs): contains the SPA entry point, `init`
...
5. Clone this repo and open a terminal in the `reframe-app` project root directory
...
The `debug?` variable in [`config.cljs`](src/cljs/reframe_app/config.cljs) defaults to `true` in
[`dev`](#running-the-app) builds, and `false` in [`prod`](#production) builds.

Use `debug?` for logging or other tasks that should run only on `dev` builds:

```clj
(ns reframe-app.example
  (:require [reframe-app.config :as config])
